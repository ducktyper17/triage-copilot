...
import { clampConfidence, defaultModelFor, loadLLMSettings, normalizeAction, runLLMVerdict, safeJsonParse, } from "./llm.js";
...
describe("runLLMVerdict", () => {
    it("returns disabled verdict when provider is none", async () => {
        const context = createMockContext({
            settings: {
                get: async (key) => key === "llm-provider" ? "none" : "",
            },
        });
    ...
    it("returns disabled verdict when API key is missing", async () => {
        const context = createMockContext({
            settings: {
                get: async (key) => key === "llm-provider" ? "groq" : "",
            },
        });
        const verdict = await runLLMVerdict(sampleIntel, context);
        expect(verdict.available).toBe(false);
