...
function dayKey(subredditName) {
    const d = new Date();
    const ymd = `${d.getUTCFullYear()}${String(d.getUTCMonth() + 1).padStart(2, "0")}${String(d.getUTCDate()).padStart(2, "0")}`;
    return `triage:actions:${subredditName}:${ymd}`;
}
function digestStatsKey(subredditName) {
    return `triage:digest-stats:${subredditName}`;
}
export async function recordModAction(subredditName, modUsername, context) {
    ...
    try {
        ...
        console.warn("[triage] mod-action count failed", e.message);
    }
}
export async function getModLoadsToday(subredditName, context) {
    const key = dayKey(subredditName);
    try {
        ...
        console.warn("[triage] mod loads read failed", e.message);
        return [];
    }
}
export function formatLoadBalancer(loads, selfUsername) {
    if (loads.length === 0)
        return null;
...
export async function getDigestStats(subredditName, context) {
    let blocked = 0;
    try {
    ...
    let triaged = 0;
    try {
        const t = await context.redis.hGet(digestStatsKey(subredditName), "triaged");
...
export async function resetWeeklyDigestCounter(subredditName, context) {
    try {
        await context.redis.hSet(digestStatsKey(subredditName), { triaged: "0" });
    }
    catch (e) {
        console.warn("[triage] digest reset failed", e.message);
    }
}
