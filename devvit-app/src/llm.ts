...
import type { CopilotFormDefaults, CopilotIntel } from "./types.js";

export type LLMVerdict = {
  available: boolean;
  tldr: string;
  suggestedAction: "approve" | "remove" | "review";
...
type LLMSettings = {
  provider: "anthropic" | "openai" | "groq" | "none";
  apiKey: string;
  model: string;
  subRules: string;
};

export function defaultModelFor(provider: LLMSettings["provider"]): string {
  switch (provider) {
    case "anthropic":
...
export async function loadLLMSettings(
  ...
  const provider = ((await context.settings.get<string>("llm-provider")) ??
    "none") as LLMSettings["provider"];
  const apiKey = (await context.settings.get<string>("llm-api-key")) ?? "";
  const configuredModel =
    (await context.settings.get<string>("llm-model")) ?? "";
  const model = configuredModel.length > 0
    ? configuredModel
    : defaultModelFor(provider);
  const subRules =
    (await context.settings.get<string>("sub-rules-text")) ?? "";
...
export function normalizeAction(v: unknown): LLMVerdict["suggestedAction"] {
  const s = String(v ?? "").toLowerCase();
  if (s === "approve" || s === "remove" || s === "review") return s;
  return "review";
}

function buildUserPrompt(intel: CopilotIntel): string {
...
export type CopilotExtras = {
  userFacingReason: string | null;
  modmailDraft: string | null;
};

export async function generateCopilotExtras(
  intel: CopilotIntel,
  verdict: LLMVerdict,
  context: TriageContext
): Promise<CopilotExtras> {
  ...
  let modmailDraft: string | null = null;

  try {
    userFacingReason = (
      await callLLMText(
        settings,
        "Write a brief, respectful removal reply for the user (2-3 sentences). Cite the rule. No markdown headers.",
        `${base}\n\nWrite the distinguished mod reply text only.`
      )
    ).trim().slice(0, 500);
  } catch (e) {
    console.warn("[triage] AI reason failed", (e as Error).message);
  }
  ...
  try {
    modmailDraft = (
      await callLLMText(
        settings,
        "Draft a private modmail to a banned user. Be clear, neutral, mention rule and appeal path.",
        `${base}\n\nBan context: temporary ban from r/${intel.input.subredditName}.\nWrite modmail body only (no subject line).`
      )
    ).trim().slice(0, 800);
  } catch (e) {
    console.warn("[triage] modmail draft failed", (e as Error).message);
  }

  return { userFacingReason, modmailDraft };
}

export function buildTrustAiDefaults(verdict: LLMVerdict): CopilotFormDefaults {
  const highTrust =
    verdict.available &&
...
export async function runLLMVerdict(
  intel: CopilotIntel,
  context: TriageContext
): Promise<LLMVerdict> {
  const settings = await loadLLMSettings(context);
  ...
  try {
    ...
    return {
      ...
      ruleHint:
        typeof parsed.ruleHint === "string" && parsed.ruleHint.length > 0
          ? parsed.ruleHint
          : null,
