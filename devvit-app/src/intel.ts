...
export async function gatherUserIntel(
  username: string,
  ...
  const recentActionsAgainstUser: string[] = [];
  let priorBansInSub = 0;
  let lastBanTimestamp: number | null = null;
  ...
  try {
    ...
    for await (const note of notesListing) {
      if (note.userNote?.note) modNotes.push(note.userNote.note);
      ...
      if (modNotes.length + recentActionsAgainstUser.length >= 20) break;
    }
  } catch (e) {
    console.warn("[triage] mod notes fetch failed", (e as Error).message);
  }
...
export async function gatherReportIntel(
  input: CopilotInput,
  context: TriageContext
): Promise<ReportIntel | null> {
  try {
    const target =
    ...
    console.warn("[triage] report intel fetch failed", (e as Error).message);
    return null;
  }
}

async function gatherBrigadeIntel(
  username: string,
  subredditName: string,
  context: TriageContext
): Promise<BrigadeIntel> {
  const empty: BrigadeIntel = {
    flagged: false,
    matchedSubs: [],
  ...
  const raw =
    (await context.settings.get<string>("brigade-watch-subs")) ?? "";
  const watchList = raw
    .split(/[\n,]+/)
  ...
  try {
    ...
    console.warn("[triage] brigade scan failed", (e as Error).message);
    return empty;
  }
...
async function gatherActivitySparkline(
  username: string,
  subredditName: string,
  context: TriageContext
): Promise<ActivitySparkline> {
  const dayCounts = new Array(SPARKLINE_DAYS).fill(0) as number[];
  const labels: string[] = [];
  const now = Date.now();
  ...
  try {
    ...
    if (user) {
      ...
      for await (const c of comments) {
        if (
          (c.subredditName ?? "").toLowerCase() !==
          subredditName.toLowerCase()
        ) {
          continue;
        }
        const ageDays = Math.floor(
          (now - c.createdAt.getTime()) / DAY_MS
        );
    ...
    console.warn("[triage] sparkline failed", (e as Error).message);
  }
...
export async function gatherRemovalTemplates(
  subredditName: string,
  context: TriageContext
): Promise<string[]> {
  ...
  try {
    ...
    for (const r of reasons) {
      ...
      if (msg.length > 0 && !templates.includes(msg)) templates.push(msg);
      if (templates.length >= 8) break;
    }
  } catch (e) {
    console.warn("[triage] removal reasons failed", (e as Error).message);
  }
  ...
  if (templates.length === 0) {
    templates.push(
      ...
      "Spam / off-topic.",
      "Harassment or hate — not allowed here."
    );
  }
...
export async function gatherCopilotIntel(
  input: CopilotInput,
  context: TriageContext
): Promise<CopilotIntel> {
  const [
    user,
    report,
    autoMod,
...
export function formatBrigadeWarning(brigade: BrigadeIntel): string | null {
  if (!brigade.flagged) return null;
  return [
    "🚨 BRIGADE WATCH",
...
export function formatSparkline(sparkline: ActivitySparkline): string {
  const activity =
    sparkline.totalInSub === 0
