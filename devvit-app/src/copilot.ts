...
import {
  formatBrigadeWarning,
  formatSparkline,
} from "./intel.js";
import type { LLMVerdict } from "./llm.js";
import { formatSimilarRemovals } from "./similar.js";
import type {
  CopilotActionChoices,
  CopilotInput,
  CopilotIntel,
} from "./types.js";

export function formatIntelSummary(
  intel: CopilotIntel,
  verdict: LLMVerdict | null,
  extras?: { userFacingReason?: string | null; modmailDraft?: string | null }
): string {
  ...
  lines.push(
    `Account ${user.accountAgeDays}d old  ·  karma: ${user.karmaInSub}`
  );

  if (user.priorBansInSub > 0) {
    const lastBan =
      user.lastBanDaysAgo === null
        ? ""
  ...
  if (verdict && verdict.available) {
    ...
    lines.push(
      `🤖 ${verdict.suggestedAction.toUpperCase()} (${confPct}% conf)`
    );
    if (verdict.ruleHint) lines.push(`   rule: ${verdict.ruleHint}`);
...
export function resolveRemovalReason(
  templateChoice: string,
  manualReason: string,
  useAiReason: boolean,
  aiReason: string
): string {
  if (useAiReason && aiReason.trim().length > 0) return aiReason.trim();
  if (manualReason.trim().length > 0) return manualReason.trim();
