...
import {
  buildTrustAiDefaults,
  generateCopilotExtras,
  runLLMVerdict,
} from "./llm.js";
...
import type { CopilotInput, CopilotIntel } from "./types.js";
...
Devvit.addSettings([
  {
    ...
    helpText:
      "Groq runs open-source Llama models with a free tier and is the recommended default.",
    defaultValue: ["groq"],
    options: [
  ...
  },
  {
    ...
    helpText:
      "Configured via `devvit settings set llm-api-key`. Encrypted at rest.",
    isSecret: true,
    scope: "app",
  },
  {
    ...
    helpText:
      "Defaults: llama-3.3-70b-versatile (Groq), claude-haiku-4-5-20251001 (Anthropic), gpt-4o-mini (OpenAI).",
    scope: "installation",
  },
  {
    type: "paragraph",
    ...
    helpText:
      "Paste your sub's rules. The Copilot uses this to map violations to specific rules.",
    scope: "installation",
  },
  {
    type: "paragraph",
    ...
    helpText:
      "Subreddit names to flag when the author was recently active there (e.g. hate, brigade sources).",
    scope: "installation",
  },
]);
...
type CopilotFormData = {
  ...
  modOptions: Array<{ label: string; value: string }>;
  defaults: {
    remove: boolean;
    banDays: number;
...
async function getModeratorOptions(
  ...
  try {
    ...
    for await (const mod of mods) {
      ...
      if (name.length === 0 || name === selfUsername) continue;
      options.push({ label: `u/${name}`, value: name });
      if (options.length >= 16) break;
    }
  } catch (e) {
    console.warn("[triage] moderators list failed", (e as Error).message);
  }
  return options;
}

async function buildCopilotSummary(
  input: CopilotInput,
  context: TriageContext,
  claimBanner: string | null,
  viewerWarning: string | null,
  nextQueueHint: string | null,
  loadHint: string | null,
  chatBlock: string | null
): Promise<{
  intelSummary: string;
  intel: CopilotIntel;
  verdict: Awaited<ReturnType<typeof runLLMVerdict>>;
  extras: Awaited<ReturnType<typeof generateCopilotExtras>>;
}> {
  ...
  const parts: string[] = [];
  if (claimBanner) parts.push(claimBanner);
  if (viewerWarning) parts.push(viewerWarning);
  if (nextQueueHint) parts.push(nextQueueHint);
  if (loadHint) parts.push(loadHint);
...
const copilotForm = Devvit.createForm(
  (data) => {
    const d = data as CopilotFormData;
    const intelSummary =
      typeof d.intelSummary === "string" ? d.intelSummary : "(no intel)";
    ...
    return {
      ...
      fields: [
        ...
        },
        {
          name: "overrideClaim",
          label:
            "Override another mod's claim (only if you're sure they're not acting)",
          type: "boolean",
          defaultValue: false,
        },
      ],
  ...
  async (event, context) => {
    const data = event.values as {
      ...
      removalReason: string;
      lockThread: boolean;
      sendModmailOnBan: boolean;
      modChatMessage: string;
      handoffToMod: string[];
    ...
    const targetKind: "post" | "comment" = context.commentId
      ? "comment"
      : "post";
    ...
    const input: CopilotInput = {
      ...
      bodyText:
        "body" in target && typeof target.body === "string"
          ? target.body
    ...
    if (
      ...
      const next = await getNextQueueItem(
        subreddit.name,
        targetId,
        context
      );
    ...
    const removalReason = resolveRemovalReason(
      templateChoice,
      String(data.removalReason ?? ""),
      Boolean(data.useAiReason),
      aiReason
    );
...
async function openCopilot(
  ...
  const targetId =
    targetKind === "comment" ? context.commentId : context.postId;
  if (!targetId) {
  ...
  const [otherViewers, claimStatus, nextItem, modLoads, modOptions, chat] =
    await Promise.all([
  ...
  const { intelSummary, intel, verdict, extras } = await buildCopilotSummary(
    input,
    context,
    claimStatus.banner,
    viewerWarning,
...
const activityForm = Devvit.createForm(
  (data) => {
    const summary =
      typeof data.summary === "string" ? data.summary : "(no data)";
    return {
...
Devvit.addMenuItem({
  ...
  description:
    "See mods online, claims, collisions prevented, and today's triage counts.",
  location: "subreddit",
  forUserType: "moderator",
  onPress: async (_, context) => {
    const me = await context.reddit.getCurrentUser();
    ...
    const subreddit = await context.reddit.getCurrentSubreddit();
    const modPermissions = await me.getModPermissionsForSubreddit(
      subreddit.name
    );
    if (
      !modPermissions.includes("all") &&
...
Devvit.addSchedulerJob<WeeklyDigestData>({
...
Devvit.addTrigger({
  ...
  onEvent: async (event, context) => {
    ...
    if (postId) await markAutomodOnTarget(postId, context);
...
Devvit.addTrigger({
  ...
  onEvent: async (event, context) => {
    ...
    if (commentId) await markAutomodOnTarget(commentId, context);
    const sub = event.subreddit?.name ?? "";
    if (event.author && sub) {
      await markAutomodOnAuthor(sub, event.author, event.reason, context);
    }
